...
画图前按双目标（最小化 output_kl 与横轴 cost）再做一次非支配筛选，图上只保留 Pareto 点。
...
def filter_pareto_kl_cost(points: list[ParetoPoint]) -> list[ParetoPoint]:
    """
    双目标最小化：output_kl 与横轴 cost（默认 f_cost / bts）。

    p 支配 q ⟺ kl(p)≤kl(q) 且 cost(p)≤cost(q)，且至少一项严格更小。
    只保留非支配点，按 cost、再按 kl 升序。
    """
    ...
    for i, p in enumerate(points):
        ...
        for j, q in enumerate(points):
            ...
            if (
                q.kl <= p.kl
                and q.cost <= p.cost
                and (q.kl < p.kl or q.cost < p.cost)
            ):
                dominated = True
                break
        if not dominated:
            keep.append(p)
    keep.sort(key=lambda p: (p.cost, p.kl))
    return keep
    ...
    *,
    ...
    for method in methods:
        ...
        if not os.path.isfile(path):
            ...
            out[method["key"]] = []
            continue
        raw = load_pareto_csv(path, x_col=x_col)
        front = filter_pareto_kl_cost(raw)
        print(
            f"  {task}/{method['key']}/{split}: "
            f"{len(raw)} → Pareto(output_kl, {x_col}) {len(front)}"
        )
        out[method["key"]] = front
    return out


def pareto_envelope(
    xs: np.ndarray, values: np.ndarray
) -> tuple[np.ndarray, np.ndarray]:
    """按 cost 升序，取累积最小值（cost–metric 平面下的下包络）。"""
...
def plot_pareto_panel(
    ...
    *,
    ...
    for method in methods:
        ...
        env_d, env_y = pareto_envelope(xs, ys)
        if len(env_d):
            ax.plot(
                env_d,
                env_y,
