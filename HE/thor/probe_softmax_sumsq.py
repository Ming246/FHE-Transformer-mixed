#!/usr/bin/env python3
"""
Probe Softmax ``update_inv_D_fixed`` (Σy² round) decrypt vs poly twin.

What this block does (δ2=2 → one round)::

  inv_list = rescale(mask · inv_d)          # stored inv, still with d_delta
  exp_2u   ≈ (2·exp · inv_list · k)²        # ≈ logical y² = (exp·inv/Δ)²
  summation = rotsum(Σ exp_2u)
  new_inv   = aSOR(1/summation)             # ≈ 1/Σy² (+ new d_delta)

Poly ``thor_softmax`` / ``slot_softmax_he`` equivalent::

  y = exp · inv_σ
  y = y² · inv_Σy²

So the poly *probability* is **not** ``exp/inv``; after this round it is
``exp_2u · (new_inv / d_delta)`` (logical), matching ``y² / Σy²``.

Rem note: after σ-aSOR at start_rem=14, rem≈4 is too low for Σy²+aSOR.
This probe re-encrypts the post-σ ``exp_u`` / ``inv_d`` messages at rem=14
(working zone) with the **real decrypt range** from the prior HE step.

Usage::

  python3 HE/thor/probe_softmax_sumsq.py
  python3 HE/thor/probe_softmax_sumsq.py --start-rem 14 --level 2
"""
from __future__ import annotations

import argparse
import math
import os
import sys
import time

import numpy as np

_HERE = os.path.dirname(os.path.abspath(__file__))
_HE = os.path.abspath(os.path.join(_HERE, ".."))
_REPO = os.path.abspath(os.path.join(_HE, ".."))
for p in (_HERE, _HE, _REPO):
    if p not in sys.path:
        sys.path.insert(0, p)

from path_setup import ensure_thor_path

ensure_thor_path()


def _ensure_he_path() -> None:
    if _HE in sys.path:
        sys.path.remove(_HE)
    sys.path.insert(0, _HE)
    if _HERE not in sys.path:
        sys.path.insert(0, _HERE)


_ensure_he_path()

from engine import THOR_DEFAULT_PARAMS, create_engine, load_thor_keys  # noqa: E402
from linear_eval import dense_qk_scores, encode_score_packs_from_dense  # noqa: E402

_ensure_he_path()

from plain_softmax_delta_sim import (  # noqa: E402
    head_mask,
    plain_he_asor_ct,
    plain_update_inv_D_fixed,
)
from slot_softmax_he import SoftmaxHeParams, stockmeyer_poly_slots  # noqa: E402
from softmax_he_ct import he_asor_ct, thor_exp_ct, update_inv_D_fixed  # noqa: E402
from softmax_poly import k_key_encode_scale  # noqa: E402
from thor_encoder_linear_core import add_vec, bert_base  # noqa: E402
from slot_softmax_he import rotsum  # noqa: E402


def _encode_attention_mask_pt(engine, attention_mask: np.ndarray, *, level: int):
    attention_mask = np.asarray(attention_mask, dtype=np.float64).reshape(-1)
    n_tokens = int(np.count_nonzero(attention_mask))
    out = np.full((8,), None, dtype=object)
    for i in range(8):
        msg = np.zeros((2**15,), dtype=np.float64)
        for j in range(16):
            temp = j * (2**11)
            diag_index = i * 16 + j
            for t in range(128):
                col_index = (diag_index + t) % 128
                is_token = 1.0 if col_index < n_tokens else 0.0
                for head in range(12):
                    msg[temp + t * 16 + head] = is_token
        out[i] = engine.encode(msg, int(level))
    return out


def _mask_packs_numpy(attention_mask: np.ndarray, cfg) -> list[np.ndarray]:
    attention_mask = np.asarray(attention_mask, dtype=np.float64).reshape(-1)
    n_tokens = int(np.count_nonzero(attention_mask))
    packs = []
    for i in range(8):
        msg = np.zeros((cfg.num_slots,), dtype=np.float64)
        for j in range(16):
            temp = j * (2**11)
            diag_index = i * 16 + j
            for t in range(128):
                col_index = (diag_index + t) % 128
                is_token = 1.0 if col_index < n_tokens else 0.0
                for head in range(12):
                    msg[temp + t * 16 + head] = is_token
        packs.append(msg)
    return packs


def _plain_exp_masked(packs, mask_np, params) -> list[np.ndarray]:
    d1 = float(params.delta1)
    d2 = float(params.delta2)
    n1 = int(round(math.log2(d1)))
    scale = math.exp(params.shift / d2 / d1)
    baked = np.array(tuple(reversed(params.exp_coeffs)), dtype=np.float64) / scale
    out = []
    for p, m in zip(packs, mask_np):
        y = stockmeyer_poly_slots(baked, np.asarray(p, dtype=np.float64))
        for _ in range(n1):
            y = y * y
        out.append(y * m)
    return out


def _plain_sigma(exp_u: list[np.ndarray], cfg) -> np.ndarray:
    sigma = np.asarray(exp_u[0], dtype=np.float64).copy()
    for i in range(1, 8):
        sigma = add_vec(sigma, exp_u[i])
    return rotsum(sigma, cfg.slot_stride)


def _stats(he: np.ndarray, ref: np.ndarray, *, mask: np.ndarray | None = None) -> dict:
    a = he.ravel().astype(np.float64)
    b = ref.ravel().astype(np.float64)
    if mask is not None:
        m = mask.ravel().astype(bool)
        a, b = a[m], b[m]
    if a.size == 0:
        return {
            "err_gain1": float("nan"),
            "ls_gain": float("nan"),
            "err_ls": float("nan"),
            "corr": float("nan"),
            "median_ratio": float("nan"),
            "max_abs_he": 0.0,
            "max_abs_ref": 0.0,
        }
    denom = float(np.dot(a, a))
    if denom > 1e-30:
        g = float(np.dot(a, b) / denom)
        err_ls = float(np.max(np.abs(a * g - b)))
    else:
        g = float("nan")
        err_ls = float("inf")
    nz = np.abs(a) > 1e-8
    med = float(np.median(b[nz] / a[nz])) if np.any(nz) else float("nan")
    corr = (
        float(np.corrcoef(a, b)[0, 1])
        if a.size > 1 and np.std(a) > 1e-15 and np.std(b) > 1e-15
        else float("nan")
    )
    return {
        "err_gain1": float(np.max(np.abs(a - b))),
        "ls_gain": g,
        "err_ls": err_ls,
        "corr": corr,
        "median_ratio": med,
        "max_abs_he": float(np.max(np.abs(a))),
        "max_abs_ref": float(np.max(np.abs(b))),
    }


def _print(tag: str, s: dict) -> None:
    print(
        f"  [{tag}]\n"
        f"    err@1={s['err_gain1']:.3e}  ls_gain={s['ls_gain']:.6g}  "
        f"err@LS={s['err_ls']:.3e}  corr={s['corr']:.6f}\n"
        f"    median(ref/he)={s['median_ratio']:.6g}  "
        f"max|he|={s['max_abs_he']:.4g}  max|ref|={s['max_abs_ref']:.4g}",
        flush=True,
    )


def _rem(engine, ct) -> int:
    return int(engine.num_levels) - int(ct.level_calc)


def _ok(s: dict, *, err_tol: float = 5e-2) -> bool:
    """Near-constant y² packs make corr unstable; prefer scale + abs err."""
    if not np.isfinite(s["ls_gain"]) or abs(s["ls_gain"] - 1.0) >= 0.05:
        return False
    if s["err_gain1"] >= err_tol:
        return False
    if np.isfinite(s["corr"]) and s["corr"] > 0.99:
        return True
    # low dynamic range: median ratio near 1 is enough
    return np.isfinite(s["median_ratio"]) and abs(s["median_ratio"] - 1.0) < 0.05


def main() -> int:
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--task", default="mrpc")
    ap.add_argument("--layer", type=int, default=0)
    ap.add_argument("--level", type=int, default=2, choices=(0, 1, 2))
    ap.add_argument("--seed", type=int, default=0)
    ap.add_argument("--start-rem", type=int, default=14)
    ap.add_argument("--land-rem", type=int, default=14, help="re-encrypt rem for Σy²")
    ap.add_argument("--n-pad", type=int, default=20)
    args = ap.parse_args()

    cfg = bert_base()
    params = SoftmaxHeParams.from_softmax_poly(
        args.task, args.layer, level=args.level
    )
    d2 = float(params.delta2)
    n2 = int(round(math.log2(d2)))
    iters_r0 = int(params.asor_max_iters_sum_sq[0])

    rng = np.random.default_rng(args.seed)
    q = rng.normal(0.0, 0.05, (cfg.seq_len, cfg.hidden_dim))
    k = rng.normal(0.0, 0.05, (cfg.seq_len, cfg.hidden_dim))
    scores = np.clip(
        dense_qk_scores(q, k, cfg, scale=1.0 / np.sqrt(cfg.head_dim)), -8.0, 8.0
    )
    scores = scores * k_key_encode_scale(args.task, args.layer)
    packs = encode_score_packs_from_dense(scores, cfg)

    n_valid = cfg.seq_len - int(args.n_pad)
    attn = np.zeros(cfg.seq_len, dtype=np.float64)
    attn[:n_valid] = 1.0
    mask_np = _mask_packs_numpy(attn, cfg)
    hm = head_mask(int(cfg.num_slots), pad=0.0)
    head_m = hm > 0.5

    # ---- plain twin through σ-aSOR, then Σy² ----
    plain_exp = _plain_exp_masked(packs, mask_np, params)
    plain_sigma = _plain_sigma(plain_exp, cfg)
    plain_inv_v, plain_d_delta, plain_en = plain_he_asor_ct(
        hm,
        plain_sigma,
        float(params.e0_sigma),
        int(params.asor_max_iters_sigma),
    )
    final_inv = n2 == 1
    plain_exp2, plain_inv2_v, plain_d2, plain_en2 = plain_update_inv_D_fixed(
        plain_exp,
        mask_np,
        plain_inv_v,
        plain_d_delta,
        plain_en,
        max_iters=iters_r0,
        final_inv=final_inv,
        cfg=cfg,
    )
    # HE ``cm_mult(short)`` zero-pads (nnz≈1536); plain twin pads with 1s — match HE.
    if final_inv:
        short = np.array(([1.0] * 12 + [0.0] * 4) * (2**7), dtype=np.float64)
        he_style_mask = np.zeros(int(cfg.num_slots), dtype=np.float64)
        he_style_mask[: short.size] = short
        plain_inv2_v = plain_inv2_v * he_style_mask
    else:
        he_style_mask = np.array(
            ([1.0] * 12 + [0.0] * 4) * (2**11), dtype=np.float64
        )
    inv_keep = he_style_mask > 0.5
    pack_keep = [(m > 0.5) & inv_keep for m in mask_np]
    pack_keep_cat = np.concatenate(pack_keep)

    plain_y = [
        p * (plain_inv2_v / float(plain_d2)) * m
        for p, m in zip(plain_exp2, mask_np)
    ]

    print(
        f"=== Softmax Σy² / update_inv_D probe  {args.task} L{args.layer} "
        f"level={args.level} start_rem={args.start_rem} land_rem={args.land_rem} ===",
        flush=True,
    )
    print(
        f"delta2={d2} → n2={n2} rounds; testing round0 iters={iters_r0}  "
        f"e0_sigma={params.e0_sigma:.6g}  n_valid={n_valid}",
        flush=True,
    )
    print(
        f"plain after σ-aSOR: d_delta={plain_d_delta:.6g} en={plain_en:.6g}  "
        f"|exp|_∞={max(float(np.max(np.abs(e))) for e in plain_exp):.4g}",
        flush=True,
    )
    print(
        f"plain after Σy²: d_delta={plain_d2:.6g} en={plain_en2:.6g}  "
        f"|exp2|_∞={max(float(np.max(np.abs(e))) for e in plain_exp2):.4g}  "
        f"|y|_∞={max(float(np.max(np.abs(y))) for y in plain_y):.4g}",
        flush=True,
    )
    print(
        "poly meaning: y = exp·inv_σ  →  y ← y²·inv_Σy²  "
        "(HE: exp_2u≈y², inv≈1/Σy²; product exp_2u·inv/Δ ≈ y)",
        flush=True,
    )

    print("create engine ...", flush=True)
    t0 = time.time()
    engine = create_engine(dict(THOR_DEFAULT_PARAMS))
    keys = load_thor_keys(engine, with_bootstrap=False, with_rot_deltas=True)
    sk, pk = keys["sk"], keys["pk"]
    nl = int(engine.num_levels)
    enc_lc = nl - int(args.start_rem)
    land_lc = nl - int(args.land_rem)
    print(
        f"  ready ({time.time()-t0:.1f}s) nl={nl} enc_lc={enc_lc} land_lc={land_lc}",
        flush=True,
    )

    # ---- HE: score → exp → mask → σ → inv ----
    x_ct = np.full((8,), None, dtype=object)
    for i in range(8):
        x_ct[i] = engine.encodecrypt(
            np.asarray(packs[i], dtype=np.float64), pk, level=enc_lc
        )
    mask_pt = _encode_attention_mask_pt(
        engine, attn, level=int(x_ct[0].level_calc)
    )

    print("HE spine → σ-aSOR ...", flush=True)
    t1 = time.time()
    exp_cts = thor_exp_ct(engine, list(x_ct), params, bts_hook=None)
    exp_u = np.full((8,), None, dtype=object)
    for i in range(8):
        exp_u[i] = engine.rescale(engine.pt_ct_mult(mask_pt[i], exp_cts[i]))

    sigma_exp = exp_u[0]
    for i in range(1, 8):
        sigma_exp = engine.cc_add(sigma_exp, exp_u[i])
    sigma_exp = engine.rotsum(sigma_exp, interval=2**11)

    enc_one = engine.encodecrypt(
        hm, pk, level=int(engine.num_levels) - int(sigma_exp.level)
    )
    inv_d, d_delta, en = he_asor_ct(
        engine,
        enc_one,
        sigma_exp,
        float(params.e0_sigma),
        int(params.asor_max_iters_sigma),
        bts_hook=None,
    )
    rem_after_sigma = _rem(engine, inv_d)
    print(
        f"  done ({time.time()-t1:.1f}s) rem(inv)={rem_after_sigma}  "
        f"d_delta={d_delta:.6g} en={en:.6g}",
        flush=True,
    )

    he_exp_msgs = [
        np.asarray(engine.decrode(exp_u[i], sk, is_real=True), dtype=np.float64)
        for i in range(8)
    ]
    he_inv_msg = np.asarray(
        engine.decrode(inv_d, sk, is_real=True), dtype=np.float64
    )
    print(
        f"  HE decrypt range: |exp|_∞={max(float(np.max(np.abs(e))) for e in he_exp_msgs):.4g}  "
        f"|inv_v|_∞={float(np.max(np.abs(he_inv_msg))):.4g}",
        flush=True,
    )

    # ---- re-land at working rem for Σy² ----
    print(
        f"\nre-encrypt post-σ HE messages at rem={args.land_rem} "
        f"(σ-aSOR left rem={rem_after_sigma}, too shallow for Σy²+aSOR) ...",
        flush=True,
    )
    exp_u2 = np.full((8,), None, dtype=object)
    for i in range(8):
        exp_u2[i] = engine.encodecrypt(he_exp_msgs[i], pk, level=land_lc)
    inv_d2 = engine.encodecrypt(he_inv_msg, pk, level=land_lc)
    mask_pt2 = _encode_attention_mask_pt(engine, attn, level=land_lc)
    rem_land = _rem(engine, inv_d2)
    print(f"  land rem={rem_land}  d_delta(carry)={d_delta:.6g}", flush=True)

    print("update_inv_D_fixed (round 0, bts_hook=None) ...", flush=True)
    t2 = time.time()
    exp_2u, inv_new, d_delta_new, en_new = update_inv_D_fixed(
        engine,
        exp_u2,
        mask_pt2,
        inv_d2,
        d_delta,
        en,
        max_iters=iters_r0,
        final_inv=final_inv,
        bts_hook=None,
        rem_probe=None,
    )
    rem_inv_new = _rem(engine, inv_new)
    rem_exp2 = _rem(engine, exp_2u[0])
    print(
        f"  done ({time.time()-t2:.1f}s) rem land→exp2/inv  "
        f"{rem_land}→{rem_exp2}/{rem_inv_new}  "
        f"d_delta={d_delta_new:.6g} en={en_new:.6g}",
        flush=True,
    )
    print(
        f"  plain twin d_delta={plain_d2:.6g}  "
        f"ratio HE/plain={d_delta_new / plain_d2 if plain_d2 else float('nan'):.6g}",
        flush=True,
    )

    print("\n=== decrypt exp_2u (≈ y² stored) ===", flush=True)
    he_exp2 = [
        np.asarray(engine.decrode(exp_2u[i], sk, is_real=True), dtype=np.float64)
        for i in range(8)
    ]
    s_exp2 = _stats(
        np.concatenate(he_exp2),
        np.concatenate(plain_exp2),
        mask=pack_keep_cat,
    )
    _print("exp_2u vs plain_update (mask∩final_inv keep)", s_exp2)

    print("\n=== decrypt new inv_d (stored) ===", flush=True)
    he_inv2_v = np.asarray(
        engine.decrode(inv_new, sk, is_real=True), dtype=np.float64
    )
    s_inv_stored = _stats(he_inv2_v, plain_inv2_v, mask=inv_keep)
    _print("inv_d vs plain stored (final_inv keep slots)", s_inv_stored)

    he_inv2_log = he_inv2_v / float(d_delta_new)
    plain_inv2_log = plain_inv2_v / float(plain_d2)
    s_inv_log = _stats(he_inv2_log, plain_inv2_log, mask=inv_keep)
    _print("inv/d_delta vs plain logical (final_inv keep)", s_inv_log)

    print("\n=== product exp_2u · (inv/d_delta) ≈ poly y ===", flush=True)
    he_y = [he_exp2[i] * he_inv2_log * mask_np[i] for i in range(8)]
    s_y = _stats(
        np.concatenate(he_y),
        np.concatenate(plain_y),
        mask=pack_keep_cat,
    )
    _print("HE product vs plain Δ-sim y (mask∩final_inv keep)", s_y)

    # naive "exp/inv" on pre-Σy² messages — wrong target, for clarity
    naive = [
        he_exp_msgs[i] / np.maximum(he_inv_msg, 1e-30) * mask_np[i]
        for i in range(8)
    ]
    s_naive = _stats(
        np.concatenate(naive),
        np.concatenate(plain_y),
        mask=pack_keep_cat,
    )
    _print("control: raw exp/inv_v (pre-Σy²) vs y — expect FAIL", s_naive)

    ok_exp2 = _ok(s_exp2, err_tol=5e-2)
    ok_inv = _ok(s_inv_log, err_tol=5e-2)
    ok_y = _ok(s_y, err_tol=5e-2)
    print("\n=== verdict ===", flush=True)
    print(
        f"  exp_2u={'PASS' if ok_exp2 else 'FAIL'}  "
        f"inv/Δ={'PASS' if ok_inv else 'FAIL'}  "
        f"product≈poly_y={'PASS' if ok_y else 'FAIL'}",
        flush=True,
    )
    print(
        "  compare to poly with: decrypt(exp_2u) * decrypt(inv)/d_delta  "
        "(not exp/inv).",
        flush=True,
    )
    return 0 if (ok_exp2 and ok_inv and ok_y) else 2


if __name__ == "__main__":
    raise SystemExit(main())
